            ...
            <?php foreach ($data_gaji as $key => $value): ?>
              <tr>
                ...
                <td>
                  ...
                  if ($status): ?>
                    <?php if ($status->status=='diterima'): ?>
                      <a href="<?= base_url('admin/laporan_gaji/print_gaji/'.$guru->id_pegawai.'/'.$value->tahun.'/'.$value->bulan) ?>" class="btn btn-sm btn-success" target="_blank"><i class="fas fa-print"></i></a>
                    <?php else: ?>
                      PROSES PENGAJUAN
                    <?php endif ?>
                  <?php else: ?>
                    <a href="<?= base_url('admin/laporan_gaji/ajukan_gaji/'.$guru->id_pegawai.'/'.$value->tahun.'/'.$value->bulan) ?>" class="btn btn-sm btn-success">AJUKAN</a>
                  <?php endif ?>
                </td>
              </tr>
            <?php endforeach ?>
